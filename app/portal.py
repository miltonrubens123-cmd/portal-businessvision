...
def render_status_badge(status):
    ...
    styles = {
        ...
        "Em atendimento": {
            ...
            "text": "#0F4C45",
            "icon": "#148777",
            "svg": '<svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M12 3L4 7V12C4 17 7.5 20.5 12 21C16.5 20.5 20 17 20 12V7L12 3Z" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/></svg>',
        },
        "Aguardando cliente": {
            "bg": "#FFF6E8",
            "border": "#F2D7A6",
            "text": "#6B4E16",
            "icon": "#C58A1C",
            "svg": '<svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M12 8V12" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><circle cx="12" cy="16" r="1" fill="currentColor"/><path d="M10.29 3.86L1.82 18A2 2 0 0 0 3.53 21H20.47A2 2 0 0 0 22.18 18L13.71 3.86A2 2 0 0 0 10.29 3.86Z" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/></svg>',
        },
        "Concluído": {
            "bg": "#EDF7ED",
            "border": "#CBE7CB",
            "text": "#1F5A2E",
            "icon": "#2F8F46",
            "svg": '<svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M20 6L9 17L4 12" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>',
        },
    }
    ...
    return (
        ...
        f'border-radius:999px;background:{s["bg"]};border:1px solid {s["border"]};'
        f'color:{s["text"]};font-size:13px;font-weight:600;line-height:1;white-space:nowrap;">'
        f'<span style="display:flex;color:{s["icon"]};">{s["svg"]}</span>'
        f'<span>{status}</span>'
        f'</div>'
    )
...
def render_status_legend():
    html = (
        ...
        f'{render_status_badge("Em análise")}'
        f'{render_status_badge("Em atendimento")}'
        f'{render_status_badge("Aguardando cliente")}'
        f'{render_status_badge("Concluído")}'
        '</div>'
    )
    st.markdown(html, unsafe_allow_html=True)


def render_boolean_badge(ativo):
    if ativo:
        return (
            '<div style="display:inline-flex;align-items:center;gap:8px;padding:6px 12px;'
            'border-radius:999px;background:#EDF7ED;border:1px solid #CBE7CB;'
            'color:#1F5A2E;font-size:13px;font-weight:600;line-height:1;white-space:nowrap;">'
            '<span style="display:flex;color:#2F8F46;">'
            ...
            '</span><span>Ativo</span></div>'
        )
    return (
        '<div style="display:inline-flex;align-items:center;gap:8px;padding:6px 12px;'
        'border-radius:999px;background:#FFF1F2;border:1px solid #F5C2C7;'
        'color:#7F1D1D;font-size:13px;font-weight:600;line-height:1;white-space:nowrap;">'
        '<span style="display:flex;color:#B42318;">'
...
def aplicar_estilo_app():
    st.markdown(
        ...
        section[data-testid="stSidebar"] {
            background: linear-gradient(180deg, #081225 0%, #0D1B36 100%);
...
st.markdown("<hr style='border:1px solid #D9E2EC; margin-top:0;'>", unsafe_allow_html=True)
    ...
    if clientes:
        for cli in clientes:
            ...
            with st.container(border=True):
                ...
                with col4:
                    st.markdown(render_boolean_badge(bool(cli["ativo"])), unsafe_allow_html=True)
